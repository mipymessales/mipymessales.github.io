...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !SqlInjectionUtils::checkSqlInjectionAttempt($_POST) && isset($_POST["id_combo"])){
    ...
    $query="DELETE FROM combos WHERE id=:id_bedida";
...
$radio = $_POST["radio"];
$imagess=$_POST["imagecombo"]??"blank1.jpg";
    $id_bebida=$_POST["id"];
...
if (isset($_POST["insertar"])){
    $carrito = json_decode($_POST['carrito'] ?? '[]', true);
        ...
        $sentencia = $base_de_datos->prepare("INSERT INTO combos (productos,
nombre,
descripcion,
stock,
disponible,
...
foto,
expira,
restaurantid) VALUES (:productos, :nombrep, :descripcion, :cantidad, :disponible,:precioventa,:montodescuento,:descuento,'5', :image,:expira, :idrestaurant)");
    $productos=json_encode($carrito);
    $sentencia->bindParam(':productos',$productos);
    $cantidad = $_POST["cantidad"];
    $sentencia->bindParam(':cantidad',$cantidad);


        $precioventa = $_POST["montototal"];
    $sentencia->bindParam(':precioventa',$precioventa);

        $montototaldescuento = $_POST["montototaldescuento"];
        $sentencia->bindParam(':montodescuento',$montototaldescuento);
